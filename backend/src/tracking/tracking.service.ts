...
export class TrackingService {
  ...
  async receivePosition(
    ...
    this.logger.log(
      `Position reçue → tracker=${dto.tracker_id} colis=${dto.ref_colis} [${dto.latitude}, ${dto.longitude}]`,
    );
    ...
    const pos = this.positionRepo.create({
      ...
      timestamp_gps: dto.timestamp_gps
        ? new Date(dto.timestamp_gps)
        : new Date(),
    });
  ...
  async getLastPosition(ref_colis: string): Promise<any> {
    ...
    if (!pos)
      throw new NotFoundException(
        `Aucune position trouvée pour le colis ${ref_colis}`,
      );
    return pos;
  }
