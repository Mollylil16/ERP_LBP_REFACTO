...
export class AnalyticsService {
    ...
    async getStrategicRecommendations(): Promise<any[]> {
        ...
        const [volumeModel, unpaidModel, cashAnomalyModel, pricingModel] = await Promise.all([
            this.runVolumeModel(),
        ...
        const recommendations = [volumeModel, unpaidModel, cashAnomalyModel, pricingModel]
            .filter(Boolean)
        ...
        if (recommendations.length === 0) {
            recommendations.push({
                ...
                description: 'Les indicateurs sont stables, aucune action critique immédiate.',
                ...
                priority: 10,
                actions: [
                    { code: 'OPEN_DAILY_BRIEF', label: 'Ouvrir briefing opérationnel', route: '/dashboard' },
                ],
            });
        }
    ...
    async getRealProfitability(params: {
        ...
        const avgRecoveryDelayDays = paidRows.length
            ? Number((paidRows.reduce((s, r) => s + r.recoveryDelayDays, 0) / paidRows.length).toFixed(2))
            : 0;

        const cohortes = {
            d30: {
                count: rows.filter((r) => r.recoveryDelayDays <= 30).length,
                amount: rows.filter((r) => r.recoveryDelayDays <= 30).reduce((s, r) => s + r.revenue, 0),
            },
            d60: {
                count: rows.filter((r) => r.recoveryDelayDays > 30 && r.recoveryDelayDays <= 60).length,
                amount: rows.filter((r) => r.recoveryDelayDays > 30 && r.recoveryDelayDays <= 60).reduce((s, r) => s + r.revenue, 0),
            },
            d90plus: {
                count: rows.filter((r) => r.recoveryDelayDays > 60).length,
                amount: rows.filter((r) => r.recoveryDelayDays > 60).reduce((s, r) => s + r.revenue, 0),
            },
        };
        ...
        return {
            summary: {
                ...
                margin_pct: totalRevenue > 0 ? Number(((totalMargin / totalRevenue) * 100).toFixed(2)) : 0,
            ...
            marge_unitaire: rows.slice(0, 500).map((r) => ({
                ...
                marge_pct: r.unitRevenue > 0 ? Number(((r.unitMargin / r.unitRevenue) * 100).toFixed(2)) : 0,
    ...
    async simulatePricingScenario(params: {
        ...
        return {
            ...
            baseline: {
                ...
                costs: baseCost,
                margin: baseMargin,
                margin_pct: baseRevenue > 0 ? Number(((baseMargin / baseRevenue) * 100).toFixed(2)) : 0,
            },
            projected: {
                revenue: Number(projectedRevenue.toFixed(2)),
                costs: Number(projectedCost.toFixed(2)),
                margin: Number(projectedMargin.toFixed(2)),
                margin_pct: projectedRevenue > 0 ? Number(((projectedMargin / projectedRevenue) * 100).toFixed(2)) : 0,
            },
            impact: {
    ...
    async getModelMonitoring() {
        ...
        const avgDrift = models.length
            ? Number((models.reduce((s, m) => s + m.drift_score, 0) / models.length).toFixed(3))
            : 0;
    ...
    private async runVolumeModel(): Promise<any | null> {
        ...
        const current = await this.colisRepository.count({ where: { created_at: MoreThanOrEqual(start30) } });
        const previous = await this.colisRepository.count({ where: { created_at: Between(prev30, start30) } });
        ...
        return {
            ...
            type,
            title: isRisk ? 'Risque baisse volume (V1)' : isOpportunity ? 'Hausse volume détectée (V1)' : 'Volume global stable (V1)',
            description: `Evolution 30j: ${deltaPct.toFixed(1)}% (période actuelle vs précédente).`,
            cause: isRisk ? 'Décélération des expéditions observée sur les 30 derniers jours.' : 'Variation contenue du flux colis.',
            action: isRisk
            ...
            explanation: {
                factors: [`current_30d=${current}`, `previous_30d=${previous}`, `delta_pct=${deltaPct.toFixed(2)}`],
                confidence: 0.68,
            },
            metrics: {
            ...
            actions: isRisk
                ? [
                    { code: 'OPEN_RAPPORTS', label: 'Voir rapport historique', route: '/statistiques/historiques' },
                    { code: 'OPEN_TARIFS', label: 'Ajuster tarifs', route: '/settings/tarifs' },
                ]
                : [{ code: 'OPEN_DASHBOARD', label: 'Suivre dashboard', route: '/dashboard' }],
        };
    }
    ...
    private async runUnpaidRiskModel(): Promise<any> {
        ...
        const impayes = factures.filter((f) => Number(f.montant_paye || 0) < Number(f.montant_ttc || 0));
        const unpaidRate = (impayes.length / total) * 100;
        const unpaidAmount = impayes.reduce(
            (s, f) => s + Math.max(0, Number(f.montant_ttc || 0) - Number(f.montant_paye || 0)),
            0,
        );
        const severity = unpaidRate > 25 ? 'error' : unpaidRate > 12 ? 'warning' : 'info';
        ...
        return {
            ...
            actions: [
                { code: 'OPEN_PAIEMENTS', label: 'Ouvrir suivi paiements', route: '/paiements' },
    ...
    private async runCashAnomalyModel(): Promise<any> {
        ...
        const type = anomalyScore >= 70 ? 'error' : anomalyScore >= 40 ? 'warning' : 'info';
        ...
        return {
            ...
            actions: [
                { code: 'OPEN_CAISSE', label: 'Ouvrir suivi caisse', route: '/caisse/suivi' },
            ],
        };
    }

    private async detectCaisseAnomalySignals(): Promise<{ score: number; reasons: string[] }> {
        const now = new Date();
        ...
        const highOps = weekMovements.filter((m: any) => Number(m.montant || 0) >= 100000).length;
        if (highOps >= 3) {
            score += 30;
    ...
    private async runPricingRecommendationModel(): Promise<any> {
        ...
        const recommendation = marginPct < 18
        ...
        return {
            ...
            metrics: {
                ...
                threshold: 0,
                status: Number(scenario?.impact?.delta_margin || 0) >= 0 ? 'ok' : 'alert',
            },
            drift: {
                status: 'stable',
                score: 0.05,
            ...
            priority: 60,
            actions: [
                { code: 'OPEN_RENTABILITE', label: 'Ouvrir rentabilité', route: '/statistiques/rentabilite' },
                { code: 'OPEN_TARIFS', label: 'Ajuster les tarifs', route: '/settings/tarifs' },
            ],
            scenario,
        };
    }
    ...
    private async loadProfitabilityRows(params: {
        ...
        if (params.date_debut) {
            query.andWhere('facture.date_facture >= :dateDebut', { dateDebut: params.date_debut });
        }
        if (params.date_fin) {
            query.andWhere('facture.date_facture <= :dateFin', { dateFin: params.date_fin });
        }
        if (params.agence_id) {
        ...
        for (const f of factures) {
            ...
            const unpaidAmount = Math.max(0, Number(f.montant_ttc || 0) - Number(f.montant_paye || 0));

            const marchandises = (f.colis as any)?.marchandises || [];
            const poidsTotal = marchandises.reduce((s: number, m: any) => s + Number(m.poids_total || 0), 0);
            const coutDirect = marchandises.reduce((s: number, m: any) => s + Number(m.cout_reel || 0), 0);
            ...
            const endDate = unpaidAmount <= 0 && lastPaiement?.date_paiement
                ? new Date(lastPaiement.date_paiement as any)
                : new Date();
            const recoveryDelayDays = Math.max(0, Math.floor((endDate.getTime() - invoiceDate.getTime()) / (1000 * 60 * 60 * 24)));

            const produit = marchandises.length
                ? Array.from(new Set(marchandises.map((m: any) => m.nom_marchandise || 'Produit'))).join(', ')
    ...
    private groupPnl(
        ...
        const map = new Map<string, { key: string; revenue: number; costs: number; margin: number }>();
