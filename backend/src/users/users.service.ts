...
export class UsersService implements OnApplicationBootstrap {
    ...
    async updateUser(id: number, dto: UpdateUserDto): Promise<User> {
        ...
        if (dto.agence_id !== undefined) {
            if (dto.agence_id === null) {
                ...
                const agence = await this.agenceRepository.findOne({ where: { id: dto.agence_id } });
    ...
    async findById(id: number): Promise<User | null> {
        return this.usersRepository.findOne({ where: { id }, relations: ['agence'] });
    }
    ...
    async getUserStats(): Promise<{
        ...
        return { total: users.length, actifs, inactifs: users.length - actifs, parRole };
    }
    ...
    async createDefaultAdmin() {
        const testUsers = [
            { username: 'admin', password: 'adminpassword', nom_complet: 'Administrateur Système', role: UserRole.DIRECTEUR, code_acces: 2, agence_code: 'DG' },
            { username: 'kadjo', password: 'adminpassword', nom_complet: 'Kadjo Serge', role: UserRole.DIRECTEUR, code_acces: 2, agence_code: 'FR-PARIS' },
            { username: 'adja', password: 'password123', nom_complet: 'Adja Nadege', role: UserRole.MANAGER, code_acces: 1, agence_code: 'CI-ABOBO' },
            { username: 'adepo', password: 'password123', nom_complet: 'Adepo Marie-Estelle', role: UserRole.MANAGER, code_acces: 1, agence_code: 'CI-AEROPORT' },
            { username: 'mon', password: 'password123', nom_complet: 'Mon Bertrand', role: UserRole.AGENT_EXPLOITATION, code_acces: 1, agence_code: 'CI-AEROPORT' },
            { username: 'abia', password: 'password123', nom_complet: 'Abia Valerie', role: UserRole.CAISSIER, code_acces: 1, agence_code: 'DG' },
            { username: 'ndri', password: 'password123', nom_complet: "N'Dri Marina", role: UserRole.AGENT_GROUPAGE, code_acces: 1, agence_code: 'DG' },
            { username: 'dialy', password: 'password123', nom_complet: 'Dialy Edwige', role: UserRole.AGENT_GROUPAGE, code_acces: 1, agence_code: 'DG' },
            { username: 'kabore', password: 'password123', nom_complet: 'Kabore Nelly', role: UserRole.AGENT_SUIVI, code_acces: 1, agence_code: 'DG' },
            { username: 'akoiblin', password: 'password123', nom_complet: 'AKOIBLIN ROXANNE', role: UserRole.SUPERVISEUR_REGIONAL, code_acces: 1, agence_code: 'CI-ADJAME', special_actions: ['pageAgence', 'voirToutesAgences', 'nePeutPasSupprimer', 'uniquementGroupage', 'nePeutPasModifier', 'pageIndividuelle'] },
            { username: 'ouedraogo', password: 'password123', nom_complet: 'Ouedraogo Mondesire', role: UserRole.CAISSIER_GROUPAGE, code_acces: 1, agence_code: 'SN-DAKAR' },
        ];
    ...
    async createOrResetTestUser(userData: {
        ...
        if (userData.agence_code) {
            agence = await this.agenceRepository.findOne({ where: { code: userData.agence_code } });
        }
        ...
        if (!existingUser) {
            ...
            console.log(`✅ User created: ${userData.username} (Agence: ${userData.agence_code || 'N/A'})`);
            ...
            if (agence && (!existingUser.agence || (existingUser.agence as any).id !== agence.id)) {
    ...
    private async linkSpecialActions(user: User, actionCodes: string[]) {
        ...
        for (const code of actionCodes) {
            const action = await this.actionSpecialeRepository.findOne({ where: { code } });
            if (action) {
                const userAction = this.userActionSpecialeRepository.create({ user, actionSpeciale: action });
    ...
    async create(userData: Partial<User>): Promise<User> {
        try {
            ...
            if (!password) throw new InternalServerErrorException('Password is required');
            const hashedPassword = await bcrypt.hash(password, 10);
            const user = this.usersRepository.create({ ...rest, password: hashedPassword } as User);
            return this.usersRepository.save(user);
        } catch (error: any) {
