...
export class FacturesService {
  ...
  private async generateReference(): Promise<string> {
    ...
    const lastFacture = await this.factureRepository
      ...
      .where('facture.num_facture LIKE :pattern', {
        pattern: `FCO-${datePart}-%`,
      })
  ...
  async generatePDF(id: number, user?: any): Promise<Buffer> {
    ...
    const {
      PaiementLienService,
    } = require('../paiements/paiements-lien.service');
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const logoPath = path.join(
          publicDir,
          'images',
          'WhatsApp Image 2026-02-20 at 12.20.22.jpeg',
        );
        ...
        const frontendBase =
          process.env.FRONTEND_URL || 'https://labelleporte.net';
        ...
        doc
          ...
          .font('Helvetica')
          .fillColor(GOLD)
          .text('IMPRIMÉ SPÉCIFIQUE', TBX, y + 11, {
            width: TBW,
            align: 'center',
          });
        doc
          ...
          .font('Helvetica-Bold')
          .fillColor(WHITE)
          .text('Facture & Colisage', TBX, y + 23, {
            width: TBW,
            align: 'center',
          });
        ...
        doc
          ...
          .font('Helvetica')
          .fillColor(MEDIUM)
          .text('Suivi colis', W - M - QR_SIZE, y + QR_SIZE + 3, {
            width: QR_SIZE,
            align: 'center',
          });
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fillColor(NAVY)
          .text('Agence: ', M, y, { continued: true });
        ...
        doc
          .font('Helvetica-Bold')
          .fillColor(NAVY)
          .text(`SERVICE CLIENT : ${callCenterNumbers}`.trim(), 0, y, {
            align: 'right',
            width: W - M,
          });
        y += 14;
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fillColor(WHITE)
          .text(`DÉTAILS COLIS  ${colisRef}`, M, y + 10, {
            width: CONTENT_W,
            align: 'center',
          });
        y += 36;
        ...
        const nbreColis = (facture.colis.marchandises || []).reduce(
          (s: number, m: any) => s + Number(m.nbre_colis || 0),
          0,
        );
        doc
          .fontSize(8)
          .font('Helvetica')
          .fillColor(DARK)
          .text(`Nombre total de colis : ${nbreColis}`, M, y + 2, {
            width: CONTENT_W,
            align: 'center',
          });
        y += 16;
        ...
        const drawRow = (
          ...
          doc
            ...
            .font('Helvetica')
            .fillColor(DARK)
            .text(value, x + 95, iy, { width: INFO_W - 95 });
        };
        ...
        drawRow(
          "Date d'envoi :",
          new Date(
            facture.colis.date_envoi || facture.date_facture,
        ...
        drawRow(
          'EXPÉDITEUR :',
          (expediteur?.nom_exp || '—').toUpperCase(),
          leftX,
          y,
        );
        drawRow(
          'DESTINATION :',
          (facture.colis.lieu_dest || '—').toUpperCase(),
          rightX,
          y,
        );
        y += 12;
        ...
        drawRow(
          'DESTINATAIRE :',
          (facture.colis.nom_dest || '—').toUpperCase(),
          rightX,
          y,
        );
        y += 12;
        ...
        (facture.colis.marchandises || []).forEach((m: any, i: number) => {
          ...
          const rowData = [
            ...
            {
              val: `${formatNum(prixKg)} F`,
              key: 'prix',
              align: 'right',
              bold: false,
            },
            {
              val: `${formatNum(totalLg)} F`,
              key: 'total',
              align: 'right',
              bold: true,
            },
        ...
        const totalsRows: Array<{
          ...
          ...(totalEmb > 0
            ? [{ label: 'EMBALLAGE', value: `${formatNum(totalEmb)} FCFA` }]
            : []),
          ...(totalAss > 0
            ? [{ label: 'ASSURANCE', value: `${formatNum(totalAss)} FCFA` }]
            : []),
          ...(totalAgence > 0
            ? [
                {
                  label: 'FRAIS AGENCE',
                  value: `${formatNum(totalAgence)} FCFA`,
                  bold: true,
                },
              ]
            : []),
          {
            label: 'MONTANT TOTAL',
            value: `${formatNum(montantTTC)} FCFA`,
            hl: true,
            bold: true,
          },
        ...
        totalsRows.forEach((row) => {
          ...
          if (row.isApprox) {
            ...
            doc.font('Helvetica-Bold').text(row.value, VALUE_X, y + 2, {
              width: VALUE_W,
              align: 'right',
            });
          } else {
            doc.text(row.value, VALUE_X, y + 2, {
              width: VALUE_W,
              align: 'right',
            });
          }
        ...
        if (paymentQrBuffer) {
          ...
          // Petit cadre élégant
          doc
            .rect(payQrX - 4, payQrY - 4, PAY_QR_SIZE + 8, PAY_QR_SIZE + 24)
            .lineWidth(0.5)
            .strokeColor('#F0F0F0')
            .stroke();
          ...
          doc
            ...
            .font('Helvetica-Bold')
            .fillColor(NAVY)
            .text('SCANNEZ POUR PAYER', payQrX, payQrY + PAY_QR_SIZE + 3, {
              width: PAY_QR_SIZE,
              align: 'center',
            });
          doc
            ...
            .font('Helvetica')
            .fillColor(MEDIUM)
            .text('(Wave / Orange Money)', payQrX, payQrY + PAY_QR_SIZE + 12, {
              width: PAY_QR_SIZE,
              align: 'center',
            });
        ...
        doc
          .roundedRect(M + SIG_W + 10, y, SIG_W, SIG_H, 2)
          .lineWidth(0.6)
          .strokeColor(BORDER)
          .stroke();
        ...
        doc
          ...
          .font('Helvetica')
          .fillColor(MEDIUM)
          .text('Lun–Ven : 08h–17h  |  Sam–Dim : 08h–14h30', M, HOR_Y + 8, {
            width: HALF,
            align: 'center',
          });
        doc.text(
        ...
        const timeEdit = now.toLocaleTimeString('fr-FR', {
          hour: '2-digit',
          minute: '2-digit',
        });
        ...
        doc
          ...
          .fillColor(MEDIUM)
          .text(
            `Édité par ${editorName} le ${dateEdit} à ${timeEdit}`,
            M,
            HOR_Y + 18,
          );
        doc.text(`Réf. ${facture.num_facture}`, 0, HOR_Y + 18, {
          width: W - M,
          align: 'right',
        });
        ...
        if (footerOk) {
          const FTR_IMG_H = 35;
          doc.image(footerImgPath, 0, H - FTR_IMG_H, {
            width: W,
            height: FTR_IMG_H,
          });
        } else {
