...
export class ExpeditionsService {
  ...
  async create(createExpeditionDto: any, user: any) {
    const agenceDepart = await this.agenceRepository.findOne({
      where: { id: user.id_agence },
    });
    if (!agenceDepart) {
      throw new NotFoundException('Agence de départ non trouvée');
    }

    const agenceDestination = await this.agenceRepository.findOne({
      where: { id: createExpeditionDto.id_agence_destination },
    });
  ...
  async removeColis(id: number, colisId: number) {
    const colis = await this.colisRepository.findOne({
      where: { id: colisId },
      relations: ['expedition'],
    });
    if (
      colis &&
      colis.expedition &&
      Number(colis.expedition.id) === Number(id)
    ) {
  ...
  async updateStatus(id: number, statut: ExpeditionStatut) {
    ...
    if (
      statut === ExpeditionStatut.EN_TRANSIT &&
      oldStatut !== ExpeditionStatut.EN_TRANSIT
    ) {
