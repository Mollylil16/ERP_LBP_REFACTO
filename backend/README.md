...
### Variables d’environnement (ne pas committer `.env`)

- **Ne versionnez jamais** `backend/.env` (secrets DB + `JWT_SECRET`). Utilisez `backend/.env.example`.
- Variables importantes:
  - **`JWT_SECRET`**: chaîne longue aléatoire (rotation possible en maintenance).
  - **`JWT_EXPIRES_IN`**: durée access token (ex: `1h`). Supporte aussi `JWT_EXPIRATION` (legacy).
  - **`REFRESH_TOKEN_TTL_DAYS`**: durée refresh token (opaque, rotatif, révoqué au logout).
  - **`CORS_ORIGINS`**: allowlist stricte en prod.
  - **`FORCE_HTTPS=true`**: si TLS est terminé par un proxy (NGINX) et `x-forwarded-proto=https` est présent.

### Durcissements activés

- **Rate limiting** sur `POST /auth/login`.
- **Rotation refresh token** côté API (`POST /auth/refresh`) + révocation au `POST /auth/logout`.
- **Audit sécurité**: tentatives login (succès/échec), refresh, logout, mise à jour des permissions de rôle.
