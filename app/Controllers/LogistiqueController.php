...
final class LogistiqueController extends BaseController
{
    ...
    {
        AuthMiddleware::check();
        PermissionMiddleware::checkOperation(OperationPolicy::FINANCE_COMPENSATIONS_VIEW);

        $filters = ['status' => $_GET['status'] ?? ''];
        $credits  = $this->repo->getAllCredits($filters);
        $agencies = $this->repo->getAgencies();

        $this->view('logistique/credits/index', $this->viewData('Crédits Inter-Agences', 'credits', [
            'credits'  => $credits,
            'agencies' => $agencies,
            'filters'  => $filters,
        ]));
    ...
    {
        AuthMiddleware::check();
        PermissionMiddleware::checkOperation(OperationPolicy::FINANCE_COMPENSATIONS_MANAGE);

        $agencies = $this->repo->getAgencies();

        $this->view('logistique/credits/create', $this->viewData('Nouveau Crédit Inter-Agences', 'credits', [
            'agencies' => $agencies,
        ]));
    }
    ...
    public function storeCredit(): void
    {
        ...
        if (!Csrf::verify($_POST['_csrf_token'] ?? null)) {
            Session::flash('error', 'Session expirée.');
            $this->redirect('/logistique/credits/nouveau');
        }

        $data = [
            'from_agency_id'  => (int)($_POST['from_agency_id'] ?? 0),
            'to_agency_id'    => (int)($_POST['to_agency_id'] ?? 0),
            'amount'          => (float)($_POST['amount'] ?? 0),
            'currency'        => $_POST['currency'] ?? 'XOF',
        ...
        if ($data['from_agency_id'] <= 0 || $data['to_agency_id'] <= 0 || $data['amount'] <= 0) {
            Session::flash('error', 'Agences et montant obligatoires.');
            $this->redirect('/logistique/credits/nouveau');
        }

        $this->repo->createCredit($data);
        Session::flash('success', 'Crédit inter-agences enregistré.');
        $this->redirect('/logistique/credits');
    }
    ...
    public function apurerCredit(int $id): void
    {
        ...
        $this->repo->apurerCredit($id);
        Session::flash('success', 'Crédit marqué comme réglé.');
        $this->redirect('/logistique/credits');
    }
