<?php

declare(strict_types=1);

use App\Router;

/** @var Router $router */

require __DIR__ . '/public.php';
require __DIR__ . '/auth.php';
require __DIR__ . '/portal.php';
require __DIR__ . '/finance.php';
require __DIR__ . '/rh.php';
require __DIR__ . '/employee.php';
require __DIR__ . '/colisage.php';
require __DIR__ . '/logistique.php';
require __DIR__ . '/crm.php';
require __DIR__ . '/tickets.php';
require __DIR__ . '/site_admin.php';
require __DIR__ . '/transit_douane.php';
require __DIR__ . '/tracking_colis.php';
require __DIR__ . '/facturation.php';
require __DIR__ . '/entrepots.php';
require __DIR__ . '/flotte_transport.php';
require __DIR__ . '/portefeuille_clients.php';
require __DIR__ . '/agents_correspondants.php';
require __DIR__ . '/pilotage_dg.php';
require __DIR__ . '/admin.php';
