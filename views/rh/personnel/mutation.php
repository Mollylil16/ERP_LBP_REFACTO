...
<div class="finea-shell"><div class="finea-container">
    ...
    <form method="post" action="<?= View::url('rh/personnel/' . (int) $employee['id'] . '/mutation') ?>" class="finea-section-card rh-operation-form">
        ...
        <div class="rh-form-grid">
            ...
            <div class="finea-field"><label>Nouveau service</label><select class="finea-select" name="service_id"><option value="">Conserver</option><?php $optionsHtml($options['services'], $employee['service_id']); ?></select></div>
            <div class="finea-field"><label>Nouvelle fonction</label><select class="finea-select" name="function_id"><option value="">Conserver</option><?php $optionsHtml($options['functions'], $employee['function_id']); ?></select></div>
            <div class="finea-field"><label>Nouveau statut</label><select class="finea-select" name="status_id"><option value="">Conserver</option><?php $optionsHtml($options['statuses'], $employee['status_id']); ?></select></div>
