...
ob_start();
?>
<div class="finea-shell employee-shell"><div class="finea-container">
    ...
    <div class="employee-two-columns">
        ...
        <section class="finea-section-card"><h2 class="finea-section-title">Historique du processus</h2><ol class="employee-timeline"><?php foreach ($request['events'] as $event): ?><li><strong><?= View::e($event['status']) ?></strong><span><?= View::e($event['comment']) ?></span><time><?= $date($event['created_at']) ?></time></li><?php endforeach; ?></ol></section>
    </div>
</div></div>
