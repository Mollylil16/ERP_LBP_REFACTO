document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    items = Array.isArray(stored)
      ? stored.filter((item) => item && typeof item.name === "string")
      : [];
  } catch (_) {
    items = [];
  }
  const renderCart = () => {
    if (cartItems) {
      cartItems.innerHTML = items.length
        ? items
            .map(
              (item) =>
                `<div class="site-cart-item"><strong>${escapeHtml(item.name)}</strong><span>${formatPrice(item.price)} XOF</span></div>`,
            )
            .join("")
    ...
    if (cartTotal)
      cartTotal.textContent = `${formatPrice(items.reduce((sum, item) => sum + item.price, 0))} XOF`;
    document
      .querySelectorAll("[data-account-cart-count]")
      .forEach((element) => {
  ...
  document.querySelectorAll("[data-add-cart]").forEach((button) => {
    button.addEventListener("click", () => {
      items.push({
        name: button.dataset.product || "Offre",
  ...
  const activate = (code) => {
    ...
    markers.forEach((marker) =>
      marker.classList.toggle("is-active", marker.dataset.code === code),
    );
    cards
  ...
  const filter = () => {
    ...
    cards.forEach((card) => {
      ...
      const marker = markers.find(
        (item) => item.dataset.code === card.dataset.code,
      );
  ...
  cards.forEach((card) =>
    card.addEventListener("click", () => activate(card.dataset.code)),
  );
  markers.forEach((marker) =>
    marker.addEventListener("click", () => activate(marker.dataset.code)),
  );
