(() => {
  ...
  const cards = Array.from(
    document.querySelectorAll("[data-health-module-card]"),
  );
  ...
  const maintenanceModalMessage = document.querySelector(
    "[data-maintenance-modal-message]",
  );
  const maintenanceReasonInput = document.querySelector(
    "[data-maintenance-reason-input]",
  );
  const maintenanceConfirmButton = document.querySelector(
  ...
  function moduleInfo(card) {
    return {
      ...
      label:
        card.dataset.healthModuleLabel ||
        card.dataset.healthModuleCard ||
        "Module",
  ...
  function setBusy(value) {
    ...
    if (runAllButton) {
      ...
      runAllButton.textContent = value
        ? "Tests en cours…"
  ...
  function setFocus(slug = null) {
    ...
    if (slug) {
      document
        .querySelector(`[data-health-module-card="${CSS.escape(slug)}"]`)
  ...
  function globalStatus(payloads) {
    if (payloads.some((payload) => payload.status === "failed"))
      return "failed";
    if (payloads.some((payload) => payload.status === "warning"))
      return "warning";
    return "passed";
  }
  ...
  function renderGlobalReport() {
    ...
    const score = payloads.length
      ? Math.round(
          payloads.reduce(
            (sum, payload) => sum + Number(payload.score || 0),
            0,
          ) / payloads.length,
        )
      : 0;
    ...
    entries.forEach(({ info, payload }) => {
      ...
      const warning = (payload.checks || []).filter(
  ...
  async function runOne(info, options = {}) {
    ...
    showConsole(
      standalone
        ? `Test du module ${info.label}`
        : `Module ${index + 1}/${total} • ${info.label}`,
    ...
    const stopSimulation = simulateProgress(info, (localProgress) => {
      setCardStatus(
        info,
        "running",
        `En cours • ${Math.round(localProgress)}%`,
      );
  ...
  async function runAllSequentially() {
    ...
    showConsole(
      "Préparation du test complet",
      `${cards.length} modules seront testés à tour de rôle.`,
    );
  ...
  async function runStandalone(info) {
    ...
    setGlobalProgress(
      100,
      "Test terminé",
      `Score du module : ${Number(payload.score || 0)}%`,
    );
    showConsole(
      `Test terminé • ${info.label}`,
      `Score obtenu : ${Number(payload.score || 0)}%.`,
    );
    setBusy(false);
  }
  ...
  async function toggleMaintenance(info, reason = "") {
    ...
    if (!currentlyMaintained && reason.length < 5) {
      ...
      showConsole(
        "Motif requis",
        "Indiquez la raison de la maintenance avant de désactiver le module.",
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !payload.ok) {
        throw new Error(
          payload.message || "Impossible de modifier le mode maintenance.",
        );
      }
      ...
      const display = info.card.querySelector(
        "[data-maintenance-reason-display]",
      );
      if (display) {
      ...
      setCardStatus(
        ...
        maintenance ? "Maintenance" : "En attente",
      );
      showConsole(
        maintenance
          ? `${info.label} est en maintenance`
          : `${info.label} est de nouveau disponible`,
        maintenance
          ? savedReason
          : "Le module est accessible depuis le portail.",
      );
    } catch (error) {
  ...
  cards.forEach((card) => {
    const info = moduleInfo(card);
    card
      .querySelector("[data-health-run-module]")
      ?.addEventListener("click", () => runStandalone(info));
    card
      .querySelector("[data-health-open-details]")
      ?.addEventListener("click", () => {
        const report = reports.get(info.slug);
        openDetails(
          report ? `Rapport • ${info.label}` : `Module ${info.label}`,
          report?.payload || {
            message: "Lancez le test de ce module pour afficher son rapport.",
          },
        );
      });
    card
      .querySelector("[data-maintenance-toggle]")
      ?.addEventListener("click", () => {
