document.addEventListener("DOMContentLoaded", () => {
  ...
  const updateEmployeePreview = () => {
    ...
    ["name", "number", "email", "phone", "service", "function"].forEach((field) => {
      const target = employeePreview.querySelector(`[data-rh-field="${field}"]`);
      if (target) target.textContent = option.dataset[field] || "Non renseigné";
    });
  };
  ...
  const initialPermissions = document.querySelector("[data-initial-permissions]");
  ...
  const permissionRows = Array.from(document.querySelectorAll("[data-permission-row]"));

  permissionRows.forEach((row) => {
    const read = row.querySelector('[data-action="view"]');
    row.querySelectorAll('[data-action]:not([data-action="view"])').forEach((checkbox) => {
  ...
  document.querySelector("[data-permissions-clear]")?.addEventListener("click", () => {
    permissionRows.forEach((row) => {
  ...
  document.querySelector("[data-permissions-read]")?.addEventListener("click", () => {
    permissionRows.forEach((row) => {
      row.querySelectorAll("input[type=checkbox]").forEach((input) => {
  ...
  document.querySelector("[data-permissions-all]")?.addEventListener("click", () => {
    permissionRows.forEach((row) => {
      row.querySelectorAll("input[type=checkbox]").forEach((input) => {
        input.checked = true;
      });
